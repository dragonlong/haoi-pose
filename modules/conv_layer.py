...
import __init__
from common.plyutils import write_ply
from modules.kernel_points import load_kernels
from modules.sampling import Sample, SampleK, Group
...
def block_decider(block_name,
    ...
    elif block_name in ['simple3d',
                        'simple3d_deformable',
                        'simple3d_invariant',
                        'simple3d_equivariant',
                        'simple3d_strided',
                        'simple3d_deformable_strided',
                        'simple3d_invariant_strided',
                        'simple3d_equivariant_strided']:
        return SimpleBlock3D(block_name, in_dim, out_dim, radius, layer_ind, config)
    ...
    elif block_name in ['resnetb3d',
                        'resnetb3d_invariant',
                        'resnetb3d_equivariant',
                        'resnetb3d_deformable',
                        'resnetb3d_strided',
                        'resnetb3d_deformable_strided',
                        'resnetb3d_equivariant_strided',
                        'resnetb3d_invariant_strided']:
        return ResnetBottleneckBlockP3D(block_name, in_dim, out_dim, radius, layer_ind, config)
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, x, batch):
        if self.layer_ind == 0 and self.task == 'motion_segmentation':
            q_pts = batch.full_points[0]
            s_pts = batch.full_points[1]
            neighb_inds = batch.full_neighbors[0]        
        elif 'strided' in self.block_name:
...
class SimpleBlock3D(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        self.unary1 = UnaryBlock(out_dim, out_dim // 2, self.use_bn, self.bn_momentum, no_relu=True)

        return

    def forward(self, x, batch):      
        if 'strided' in self.block_name:
            q_pts = batch.querys[self.layer_ind + 1]
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.pools[self.layer_ind]
            s_lengths = batch.lengths[self.layer_ind+1]
        else:
            q_pts = batch.querys[self.layer_ind]
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.neighbors[self.layer_ind]
            s_lengths = batch.lengths[self.layer_ind]
        x_in = x
        print(q_pts.size(), s_pts.size(), neighb_inds.size(), x.size())
        x = self.KPConv(q_pts, s_pts, neighb_inds, x)

        x = self.leaky_relu(self.batch_norm(x))

        q_lengths = torch.sum(s_lengths.view(-1, self.window_size).contiguous(), dim=1, keepdim=True).repeat(1, self.window_size).view(-1)
        x3d_batch_flat = torch.split(x, q_lengths.cpu().numpy().tolist(), dim=0)

        x3d_batch = []
        for j in range(0, len(q_lengths), self.window_size):
            x3d = torch.cat(x3d_batch_flat[j:j+self.window_size], dim=1)
            x3d_batch.append(x3d)
        # print(f's_lengths: {s_lengths.cpu().numpy()}, q_lengths: {q_lengths.cpu().numpy()}, in: {x_in.size()}, split: {len(x3d_batch_flat)}, middle: {x.size()}')
        # breakpoint()
        x = torch.cat(x3d_batch, dim=0)

        # Second upscaling mlp
        x = self.unary1(x)
        # print(f'layer {self.layer_ind} {self.block_name} has outputs : {x.size()}')
...
class ResnetBottleneckBlockP3D(nn.Module):
    ...
    def __init__(self, block_name, in_dim, out_dim, radius, layer_ind, config):
        ...
        :param config: parameters
        """
        super(ResnetBottleneckBlockP3D, self).__init__()
        ...
        # KPConv block
        self.KPConv = nn.ModuleList()
        for i in range(1):
            self.KPConv.append(KPConv(config.num_kernel_points,
                                 config.in_points_dim,
                                 out_dim // 4,
                                 out_dim // 4,
                                 current_extent,
        ...
        # Second upscaling mlp
        self.unary2 = UnaryBlock(out_dim // 4 * self.window_size, out_dim, self.use_bn, self.bn_momentum, no_relu=True)

        # Shortcut optional mpl
        if in_dim != out_dim:
            if 'strided' in self.block_name:
                self.unary_shortcut = UnaryBlock(in_dim*self.window_size, out_dim, self.use_bn, self.bn_momentum, no_relu=True)
            else: 
                self.unary_shortcut = UnaryBlock(in_dim, out_dim, self.use_bn, self.bn_momentum, no_relu=True)
        else:
            if 'strided' in self.block_name:
    ...
    def forward(self, features, batch):
        if 'strided' in self.block_name:
            ...
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.pools[self.layer_ind]
            qs_lengths = batch.lengths[self.layer_ind+1]
        else:
            q_pts = batch.querys[self.layer_ind]
            s_pts = batch.points[self.layer_ind]
            neighb_inds = batch.neighbors[self.layer_ind]
            qs_lengths = batch.lengths[self.layer_ind]
        ...
        # use same MLP
        for t in range(1):
            x_t = self.KPConv[0](q_pts, s_pts, neighb_inds, x) # Pts is [N0+N1, N0+N1, N0'+N1', N0'+N1']
            x_t = self.leaky_relu(self.batch_norm_conv(x_t))
            feat_t.append(x_t)

        # fetch the target features
        x = feat_t[0]
        # x_conv3d = x.view(self.window_size, -1, x.size(-1)).contiguous().permute(1, 0, 2).contiguous().view(-1, x.size(-1))
        # breakpoint()
        q_lengths = torch.sum(qs_lengths.view(-1, self.window_size).contiguous(), dim=1, keepdim=True).repeat(1, self.window_size).view(-1)
        x3d_batch_flat  = torch.split(x, q_lengths.cpu().numpy().tolist(), dim=0)
        ...
        # print(f's_lengths: {qs_lengths.cpu().numpy()}, q_lengths: {q_lengths.cpu().numpy()}, in: {features.size()}, split: {len(x3d_batch_flat)}, middle: {x.size()}')
        x = torch.cat(x3d_batch, dim=0)
        ...
        # Shortcut
        if 'strided' in self.block_name:
            shortcut_t = max_pool(features, neighb_inds)
            shortcut_t  = torch.split(shortcut_t, q_lengths.cpu().numpy().tolist(), dim=0)
            shortcut_batch = []
            for j in range(0, len(q_lengths), self.window_size):
                shortcut3d = torch.cat(shortcut_t[j:j+self.window_size], dim=1)
                shortcut_batch.append(shortcut3d)
            shortcut = torch.cat(shortcut_batch, dim=0)
        else:
            shortcut = features
        shortcut = self.unary_shortcut(shortcut)
        # print(f'layer {self.layer_ind} {self.block_name} has outputs : {x.size()}')
...
if __name__ == '__main__':
    ...
    # model = pconv3d(in_channels=4, out_channels=[32], window_size=T, kernel_type='pointnet++').cuda()
    model = pconv3d(in_channels=4, out_channels=[16, 32], window_size=T, kernel_type='dpcn').cuda()
    sample_operator  = SampleK(radius=1, num_samples=16, knn=True)
    neighbor_indices = sample_operator(xyz2.view(-1, 3, N).contiguous(), xyz1.unsqueeze(1).repeat(1, T, 1, 1).view(-1, 3, N).contiguous()) # 
    # 
    print('neighbor_indices: ', neighbor_indices.size())

    x = torch.cat([feat2, xyz2], dim=2)
    new_feat = model(xyz1, xyz2, neighbor_indices, x) 
