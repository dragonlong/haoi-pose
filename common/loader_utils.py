...
import common.data_transforms
from enum import Enum, unique
from tqdm import tqdm
import __init__
from common.data_utils import IO
...
def collate_fn_obman(batch):
    batch = list(filter(lambda x: x is not None, batch))

    taxonomy_ids = []
    model_ids = []
    data = {'partial_cloud':[], 'gtcloud':[]}

    for sample in batch:
        taxonomy_ids.append(sample[0])
        model_ids.append(sample[1])
        data['partial_cloud'].append(sample[2])
        data['gtcloud'].append(sample[3])
...
class ShapeNetDataLoader(object):
    ...
    def _get_transforms(self, cfg, subset):
        if subset == DatasetSubset.TRAIN:
            return common.data_transforms.Compose([{
