...
def get_init_pose(full, partial, method='rotate_60', voxel_size=0.05):
    if method == 'global':
        ...
        centers = get_mass_center(rotated_partial)
...
def main(cfg):
    init_methods = ['rotate_60']  # ['global', 'rotate_60']
    os.chdir(hydra.utils.get_original_cwd())
    dataset = YCBDataset(cfg, cfg.DATASET.data_path, 'val')
    bs = Basic_Utils(cfg)
    err_dict = {method: {} for method in init_methods}
    ...
    for init_method in init_methods:
        ...
        for key, value in cur_dict.items():
            ...
            if key in ['rdiff', 'tdiff']:
                cur_final[f'{key}_mid'] = np.median(cur_dict[key])
            if key in ['add', 'adds']:
