...
from common.data_utils import collect_file, split_dataset, get_obj_mesh
...
class PoseDataset():
    def __init__(self, root_dir, render_path, item, second_dir=None, num_points=1024,  objs=[], add_noise=False, noise_trans=0, mode='train', refine=False, selected_list=None, is_debug=True):
        """
    ...
    def __preprocess_and_save__(self, index):
        ...
        if self.is_debug:
            ...
            # plt.show()
            obj_vertices, obj_faces = get_obj_mesh(f'{ins}_{arti_ind}_{grasp_ind}_{frame_order}', verbose=True)
            plot3d_pts([parts_cloud_cam, [joints_cam]], [['part {}'.format(i) for i in range(len(parts_cloud_cam)+1)], ['joints']], s=5, title_name=['camera coords', 'joints'], save_fig=True)
            # plot3d_pts([parts_cloud_cam, [contacts_cam]], [['part {}'.format(i) for i in range(len(parts_cloud_cam)+1)], ['contacts']], s=5, title_name=['camera coords', 'contacts'], save_fig=False)
            plot3d_pts([parts_cloud_world[:3], [joints]], [['part {}'.format(i) for i in range(len(parts_cloud_world)+1)], ['joints']], s=5, title_name=['world coords', 'joints'], save_fig=False)
...
if __name__ == '__main__':
    ...
    for j in range(len(PoseData.list_rgb)):
        dp = PoseData.__preprocess_and_save__(j)
