...
def get_intents(p_num, object_name, root_dir='data'):
  ...
  for ins in ('use', 'handoff'):
    ...
    sess_dir = osp.join(root_dir, 'contactpose_data', sess_dir, object_name)
    if osp.isdir(sess_dir):
      out.append(ins)
  return out


def get_p_nums(object_name, intent, root_dir='data'):
  """
...
class ContactPose(object):
  ...
  def __init__(self, p_num, intent, object_name, mano_pose_params=15, root_dir='data',
               load_mano=True):
    """
    ...
    self.data_dir = osp.join(root_dir, 'contactpose_data', p_id, object_name)
    ...
    oM = np.loadtxt(osp.join(root_dir, 'object_marker_locations',
