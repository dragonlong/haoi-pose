...
0.8: only train with regressionR, regressionPose, partcls loss, NOCS loss--> 6D vector directly, add vertices, joints loss
  ...
  python post_hand.py --hand --exp_num=0.7 --mano 2>&1 | tee eval_0.8_mano.log &

0.9:
python train.py verbose=True gpu=1 name_model='votenet' exp_num='0.9' MODEL.arch_decoder='votenet' pred_mano=False 2>&1 | tee train_0.9.log
python post_hand.py --hand --exp_num=0.7 --mano 2>&1 | tee eval_0.8_mano.log &
