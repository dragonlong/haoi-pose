...
import dataset.kitti.parser_patch_conv3d as parserModule
...
def main_worker(gpu, gpus, cfg, DATA, RV_CFG, args=None):
    ...
    if cfg.BEV.label_dim == 2:
        stack_factor = 1
    # get the data
    parser = parserModule.Parser(cfg=cfg)
    ...
    schedulers_dict = {}
    crit = []
    backbones['point'] = {'arch_encoder': cfg.BEV.arch_encoder.lower(),
    ...
    if args.eval: 
        # backbones['point']['weights_encoder'] = '/homes/xili/log/point_motion_lr5-3_warm_xent_pointnet2_dpcn/encoder_best_valid.pth'
        backbones['point']['weights_decoder'] = '/homes/xili/log/point_motion_lr5-3_warm_xent_pointnet2_dpcn/decoder_best_bev_val.pth'
    ...
    for epoch in range(cfg.TRAIN.start_epoch, cfg.TRAIN.num_epoch):
        ...
        if not args.eval:
            train_acc, train_iou, train_loss = train_epoch(gpu, None, modules['point'], train_loader, optimizers_dict, evaluator, epoch+1, cfg, schedulers_dict)
        ...
        val_acc, val_iou, val_loss, rand_imgs = validate_epoch(gpu, None, modules['point'], valid_loader, evaluator, parser.get_xentropy_class_string, parser.to_color, save_scans=cfg.TRAIN.save_scans)
        # update info
