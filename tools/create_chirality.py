...
if __name__ == '__main__':
    ...
    # # find the needed points
    # xyz1 = np.concatenate([xyz[xyz[:, 2]>0][:, :3], xyz[xyz[:, 2]>0][:, :3] * np.array([[1, 1, -1]])], axis=0)
    # print(f'half shape have {np.where(xyz[:, 2]>0)[0].shape[0]} pts')
    # plot3d_pts([[xyz], [xyz[xyz[:, 2]>0]], [xyz1]], [['input'], ['half'], ['chirality']], title_name=['complete', 'visible', 'visible'], s=0.2)
