...
class PoseDataset():
    def __init__(self, root_dir, item, num_points=1024,  objs=[], add_noise=False, noise_trans=0, mode='train', refine=False, selected_list=None, is_debug=False):
        ...
        self.dataset_render = second_dset + '/render'
        ...
        for ins in instances_per_obj:
            ...
            for art_index in os.listdir(base_path):
                meta[art_index] = {}
                sub_dir = base_path + '/' + art_index
                for grasp_ind in os.listdir(sub_dir):
                    sub_dir0 = sub_dir + '/' + grasp_ind
                    ...
                    try:
                        ...
                        meta[art_index][grasp_ind] = meta_instance
                    except:
                        meta[art_index][grasp_ind] = None
    ...
    def __preprocess_and_save__(self, index):
        ...
        h5_save_path       = self.root_dir + '/hdf5/' + obj_category + '/' + ins + '/' + art_status + '/' + grasp_ind
        ...
        # pose infos
        pose_dict = self.meta_dict[obj_category][ins][art_status][grasp_ind]['frame_{}'.format(frame_order)]
        urdf_dict = self.urdf_dict[obj_category][ins]
        viewMat   = np.array(pose_dict['viewMat']).reshape(4, 4).T # why we have this T
        ...
        # pick pts according to number of parts
        ...
        for s in range(num_parts):
            ...
            # !!!!!!!!!!!!!!!!!!!modify, todo, how to understand this ViewMat!!!!!!!!!!!!!!!!!!!!!!
        ...
        if self.is_debug:
            ...
            plt.show()
            plot3d_pts([parts_cloud_cam], [['part {}'.format(i) for i in range(len(parts_cloud_cam))]], s=5, title_name=['camera coords'], save_fig=True)
            plot3d_pts([parts_cloud_world], [['part {}'.format(i) for i in range(len(parts_cloud_world))]], s=5, title_name=['world coords'], save_fig=True)
            ...
            plot3d_pts([parts_cloud_urdf], [['part {}'.format(i) for i in range(len(parts_cloud_urdf))]], s=5, title_name=['urdf coords'], save_fig=True)
