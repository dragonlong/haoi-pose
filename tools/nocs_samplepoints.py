...
def create_ray_samples(sample_path,
    ...
    if os.path.exists(sample_path):
        ...
        print('not existing !!!', sample_path)
        return
...
if __name__ == "__main__":
    ...
    core_path = '/groups/CESCA-CV/ICML2021/data/nocs/obj_models/train/'
    sample_paths = []
    classes = ['00000000', '03642806', '02876657', '02954340',  '03211117']
    for class_id in classes:
        class_path = core_path + class_id
        samples = sorted(os.listdir(class_path))
        for sample in samples:
            sample_path = os.path.join(class_path, sample, '/model.obj')
    ...
    for sample in tqdm(sample_paths[args.start_idx:args.start_idx + args.group_by]):
        ...
        bp()
        if not os.path.exists(sample.replace('.obj', '_manifold.obj')):
            os.system('./manifold --input {} --output {}'.format(sample, sample.replace('.obj', '_manifold.obj')))
            print(f'---create watertight mesh takes : {time.time()-t0} sec \n', sample.replace('.pkl', '_manifold.obj'))

        # save surface points
        create_ray_samples(sample.replace('.obj', '_manifold.obj'), min_hits=100000, volumic_pts=False, surface_pts=True, cube_pts_occupancy=False, near_surface_sdf=False, verbose=True, display=False)
