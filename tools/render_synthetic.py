...
#>>>>>>>>>>>>>>>>>>>>>>---------Rendering setup----------<<<<<<<<<<<<<<<<<<<<<<<<<#
def render_data(data_root, name_obj, cur_urdf, simu_cnt, grasp_ind, joint_angles=None, args=None, cam_dis=1, urdf_file='NA', _WRITE_FLAG=True, _RENDER_FLAG=True, _CREATE_FOLDER=True, RENDER_NUM=100, ARTIC_CNT=20, _RENDER_MODE='random', _USE_GUI=True, _IS_DUBUG=True):
    #>>>>>>>>>>>>>>>>>>>>>>>>>> internal config >>>>>>>>>>>>>>>>>>>>>>#
    save_path     = '/groups/CESCA-CV/ICML2021/data' + '/render/' + name_obj
    path_urdf     = data_root + '/urdf/' + name_obj
    hand_urdf     = hand_urdf + f'/{name_obj}/{cur_urdf}/{simu_cnt}/{grasp_ind}.urdf'
    ...
    # load hand urdf
    hand_p = pybullet.loadURDF(hand_urdf)
    ...
    elif _RENDER_MODE == 'given':
        steeringAngleArray          = joint_angles
    ...
    if (not os.path.exists(save_path + '/{}/{}/{}/depth/'.format(cur_urdf, simu_cnt, grasp_ind))) and _CREATE_FOLDER:
        os.makedirs(save_path + '/{}/{}/{}/depth/'.format(cur_urdf, simu_cnt, grasp_ind))
        os.makedirs(save_path + '/{}/{}/{}/rgb/'.format(cur_urdf, simu_cnt, grasp_ind))
        os.makedirs(save_path + '/{}/{}/{}/mask/'.format(cur_urdf, simu_cnt, grasp_ind))
    yml_dict = OrderedDict()
    yml_file = save_path + '/{}/{}/{}/gt.yml'.format(cur_urdf, simu_cnt, grasp_ind)
    ...
    for i in range(view_num):
        ...
        if(img_id < RENDER_NUM and _RENDER_FLAG):
            ...
            image_path  = save_path + '/{}/{}/{}'.format(cur_urdf, simu_cnt, grasp_ind)
...
if __name__ == "__main__":
    ...
    angles_pre = np.load(f'{whole_obj}/{args.item}{args.item}.npy')
    breakpoint()
    # for i, instance in enumerate(all_ins):
    #     for arti_ind in os.listdir(hand_urdf + f'/{args.item}/{instance}/'):
    #         existing_urdfs = sorted(glob.glob( hand_urdf + f'/{args.item}/{instance}/{arti_ind}/*urdf'))
    #         print(f'go over {instance}')
    #         # breakpoint()
    #         for j, hand_urdf in enumerate(existing_urdfs):
    #             if j > 9:
    #                 break
    #             state_attrs = hand_urdf.split('.ur')[0].split('/')[-2:]
    #             simu_cnt = int(state_attrs[0])
    #             grasp_ind= int(state_attrs[1])
    #             render_data(data_root, args.item, instance, simu_cnt=simu_cnt, grasp_ind=grasp_ind, cam_dis=cam_dis, joint_angles=angles_pre[i], args=args, _CREATE_FOLDER=_CREATE, _WRITE_FLAG=_WRITE, _RENDER_FLAG=_RENDER, \
