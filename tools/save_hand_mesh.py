...
def main():
    ...
    grasps_path     = infos.grasps_path
    mano_path       = infos.mano_path
    pose_path       = infos.pose_path
    hand_path       = infos.hand_path
    ...
    json_names = glob.glob( grasps_path + '/*json') # eyeglasses_0002_0_scale_200.json
    json_names.sort()
    for json_name in json_names:
        ...
        save_dir = f'{hand_path}/{category}/{instance}/{arti_ind}'
        ...
        if args.viz:
            objname = f'{pose_path}/{category}/{instance}/{arti_ind}.obj' 
        ...
        for j in range(len(hand_attrs['grasps'])):
            ...
            with open(save_name,"w+") as fp:
                fp.write(mesh_txt)
            # mesh_dict = {'v':[hand_vertices], 'f':[hand_faces], 'n':[], 't':[]}
            # mesh_dict['f'][0] = mesh_dict['f'][0].tolist()
            # for m in range(len(mesh_dict['f'][0])):
            #     for n in range(len( mesh_dict['f'][0][m])):
            #         ind = mesh_dict['f'][0][m][n]
            #         mesh_dict['f'][0][m][n] = f'{ind}/{ind}/{ind}'
            # save_objmesh(save_name, mesh_dict, prefix=None)
            if args.viz:
