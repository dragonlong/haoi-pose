...
from common.d3_utils import compute_iou
    ...
    def eval_mesh(self, mesh, pointcloud_tgt, normals_tgt,
        ...
        if len(mesh.vertices) != 0 and len(mesh.faces) != 0:
            if remove_wall: #! Remove walls and floors
                ...
                print('target: ', np.min(pointcloud_tgt, axis=0), np.max(pointcloud_tgt, axis=0))
                print('pred mesh: ', np.min(pointcloud, axis=0), np.max(pointcloud, axis=0))
        ...
        print('final target: ', np.min(pointcloud_tgt, axis=0), np.max(pointcloud_tgt, axis=0))
        print('final pred mesh: ', np.min(pointcloud, axis=0), np.max(pointcloud, axis=0))
