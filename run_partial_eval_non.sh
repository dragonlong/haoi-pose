...
use_fps_points=True \
eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/0.913r_pre_compute_delta.txt
...
eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/0.9141r_pre_compute_delta.txt
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True \
eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/0.92r_pre_compute_delta.txt
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True \
eval=True save=True 2>&1 | tee evaluation/logs/0.92r.txt_new
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True r_method_type=1 \
eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/0.921r_pre_compute_delta.txt
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True r_method_type=1 \
eval=True save=True 2>&1 | tee evaluation/logs/0.921r.txt_new
#
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> chair <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True \
eval=True save=True 2>&1 | tee evaluation/logs/0.95r.txt_new
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True r_method_type=1 \
eval=True save=True 2>&1 | tee evaluation/logs/0.951r.txt_new
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True \
eval=True save=True 2>&1 | tee evaluation/logs/0.96r.txt_new
#
...
MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
use_fps_points=True r_method_type=1 \
eval=True save=True 2>&1 | tee evaluation/logs/0.961r.txt_new
#
# #>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> BOTTLE >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.
...
# MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
# use_fps_points=True \
# eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/pre_compute_delta_0.94r.txt_new
#
# echo 0.94r: # supervision in camera space, bottle, but add T estimation, use fps sampling,
# python train_aegan.py task='ssl_partial_pcloud_pose_completion' training=ae_gan encoder_type=enc_so3net name_model=ae vis=True save_frequency=5 \
# models=epn exp_num='0.94r' model.model='enc_so3net' model.pooling_method='max' \
...
# MODEL.num_in_channels=1 pred_t=True t_method_type=0 \
# use_fps_points=True r_method_type=1 \
# eval=True save=True pre_compute_delta=True 2>&1 | tee evaluation/logs/pre_compute_delta_0.941r.txt_new
#
# 0.941r: # corrected, supervision in camera space, random R, airplane, but add T estimation, use fps sampling, use quaternion activation, 36, 0.001
# python train_aegan.py task='ssl_partial_pcloud_pose_completion' training=ae_gan encoder_type=enc_so3net name_model=ae vis=True save_frequency=5 \
# models=epn exp_num='0.941r' model.model='enc_so3net' model.pooling_method='max' \
