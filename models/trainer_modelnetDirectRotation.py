...
class Trainer(vgtk.Trainer):
    def __init__(self, opt):
        ...
        self.summary.register(['Loss', 'reg_loss','rdiff', 'r_acc'])
    ...
    def _setup_datasets(self):
        ...
        dataloader =  NOCSDataset #Dataloader_ModelNet40 # Dataloader_ModelNet40Alignment #

        if self.opt.mode == 'train':
            dataset = dataloader(self.opt)
            self.dataset = torch.utils.data.DataLoader(dataset, \
                                                        batch_size=self.opt.TRAIN.train_batch, \
                                                        shuffle=True, \
                                                        num_workers=self.opt.num_thread)
            self.dataset_iter = iter(self.dataset)

        dataset_test = dataloader(self.opt, 'val')
        self.dataset_test = torch.utils.data.DataLoader(dataset_test, \
                                                        batch_size=self.opt.TRAIN.test_batch, \
                                                        shuffle=True, \
                                                        num_workers=self.opt.num_thread)
    ...
    def step(self):
        try:
            ...
            if data['R_label'].shape[0] < self.opt.TRAIN.train_batch:
                raise StopIteration
    ...
    def _optimize(self, data):
        ...
        in_alignment = data['R_gt'].to(self.device).float() # GT R,torch.Size([2, 3, 3])
        ...
        with torch.no_grad():
            ...
            for num in range(num_iter):
                for it, data in enumerate(self.dataset_test):
                    ...
                    self.logger.log("Testing", "Accuracy: %.1f, error: %.2f degree!"%(100*acc.item(), error.mean().item()))
            ...
            if self.opt.use_wandb:
                test_info = {'r_acc': 100 * all_acc.mean(), 'rdiff': np.mean(all_error), 'rdiff_mid': np.median(all_error), '5deg': deg5_acc}
