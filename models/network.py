...
class Network(NetworkBase):
    ...
    def train_epoch(self, gpu_id, loader, model, epoch, cfg, modules=None):
        ...
        for i, batch in enumerate(loader):
            ...
            msg = 'Loss: {loss:.3f} seg:{miou_loss:.3f} n:{nocs_loss:.3f}'.format(
                                      loss=loss_total, \
                                      ...
                                      handjoints_loss=self.total_loss_dict['handjoints_loss']) # r:{regression_loss:.3f}, R:{regressionR_loss:.3f}, T:{regressionT_loss:.3f}, hv: {handvertices_loss:.3f}, hj: {handjoints_loss:.3f}
        ...
        with torch.no_grad():
            for i, batch in enumerate(loader):
                ...
                msg = 'Loss: {loss:.3f} seg:{miou_loss:.3f} n:{nocs_loss:.3f}'.format(
                                      ...
                                      regressionT_loss=self.total_loss_dict['regressionT_loss'],\
                                      handvertices_loss=self.total_loss_dict['handvertices_loss'],\
                                      handjoints_loss=self.total_loss_dict['handjoints_loss']) # , R:{regressionR_loss:.3f}, T:{regressionT_loss:.3f}, hv: {handvertices_loss:.3f}, hj: {handjoints_loss:.3f}
