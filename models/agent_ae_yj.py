...
if 'completion' in self.config.task:
    if 'ycb' in self.config.task:
        ...
        if isinstance(self.latent_vect, dict):
            ...
            self.output_pts = self.net.decoder(self.latent_vect)
        # in nocs space
        dist1_canon, dist2_canon = self.chamfer_dist(self.output_pts.permute(0, 2, 1).contiguous(), target_pts,
                                                     return_raw=True)
        if 'partial' in self.config.task:
            self.recon_canon_loss = (dist2_canon).mean()
        else:
    ...
    np_out = self.output_pts.shape[-1]
    rotation_mapping = compute_rotation_matrix_from_quaternion if nr == 4 else compute_rotation_matrix_from_ortho6d
    pred_RAnchor = rotation_mapping(self.latent_vect['R'].transpose(1, 2).contiguous().view(-1, nr)).view(nb, -1, 3,
                                                                                                          3).float()

    anchors = torch.from_numpy(L.get_anchors(self.config.model.kanchor)).to(self.output_pts).to(
        pred_RAnchor.device).float()
    select_r = False
    ...
    if self.config.pred_t:
        ...
        transformed_pts = transformed_pts.permute(0, 1, 3, 2).contiguous()  # nb, na, np, 3
        shift_dis = input_pts.mean(dim=1, keepdim=True)
        dist1, dist2 = self.chamfer_dist(transformed_pts.view(-1, np_out, 3).contiguous(),
                                         (input_pts - shift_dis).unsqueeze(1).repeat(1, na, 1, 1).contiguous().view(-1,
                                                                                                                    N,
                                                                                                                    3).contiguous(),
                                         return_raw=True)
    else:
        transformed_pts = torch.matmul(pred_R, self.output_pts.unsqueeze(1).contiguous() - 0.5).permute(0, 1, 3,
                                                                                                        2).contiguous()  #
        dist1, dist2 = self.chamfer_dist(transformed_pts.view(-1, np_out, 3).contiguous(),
                                         input_pts.unsqueeze(1).repeat(1, na, 1, 1).contiguous().view(-1, N,
                                                                                                      3).contiguous(),
                                         return_raw=True)
