...
class NetworkBase():
    def __init__(self, gpu, cfg): # with seperate gpu for distributed training
        ...
        self.total_loss_dict['regression_loss'] = 0
        ...
        if cfg.distributed:
            for key, node_backbone in self.models.items():
    ...
    def group_weight(self, module):
        ...
        for m in module.modules():
            ...
            elif isinstance(m, nn.models.conv._ConvNd):
                group_decay.append(m.weight)
                if m.bias is not None:
                    group_no_decay.append(m.bias)
            elif isinstance(m, nn.models.batchnorm._BatchNorm):
