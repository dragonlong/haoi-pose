...
class EquivariantDGCNN(nn.Module):
    def __init__(self, k, C, C_in=1, C_out=2):
        ...
        self.m_conv1 = nn.Sequential(nn.Conv2d(self.C_in*2+1, 64, kernel_size=1, bias=True),
                                   SiLU())
        self.m_conv2 = nn.Sequential(nn.Conv2d(64*2+1, 64, kernel_size=1, bias=True),
                                   SiLU())
        self.m_conv3 = nn.Sequential(nn.Conv2d(64*2+1, 64, kernel_size=1, bias=True),
                                   SiLU())
        self.m_conv4 = nn.Sequential(nn.Conv2d(64*2+1, 64, kernel_size=1, bias=True),
                                   SiLU())
