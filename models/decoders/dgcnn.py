...
class DGCNN_semseg(nn.Module):
    def __init__(self, k=16, C_in=3, C_mid=64, num_mode=1, emb_dims=1024, num_parts=13):
        ...
        # feature fusion, fc layer and mapping!!! 1216 --> 512 --> 256 --> num_parts
    ...
    def forward(self, x):
        ...
        for key, head in self.heads.items():
            pred_dict[key] = head(x)

        return pred_dict

if __name__ == '__main__':
    # 1. try create the model
    device = torch.device("cuda:0")
    model = DGCNN_semseg(emb_dims=1024, num_parts=6).cuda()
    x = torch.rand(16, 3, 512, device=device)
    f = model(x)
    print(f, x.shape)
